...
CreateThread(function()
    ...
    ESX.TriggerServerCallback('tayer-territories:getTerritories', function(cb)
        territories = cb
    end)
...
RegisterNetEvent('tayer-territories:updateTerritories')
AddEventHandler('tayer-territories:updateTerritories', function(cb)
    territories = cb
...
RegisterNetEvent('tayer-territories:updateBlips')
AddEventHandler('tayer-territories:updateBlips', function(id, job, label)
    while true do
        Wait(1000)
...
RegisterNetEvent('tayer-territories:createTerritory')
AddEventHandler('tayer-territories:createTerritory', function()
    ...
    TriggerServerEvent('tayer-territories:createTerritory', {
        id = #territories + 1,
        name = input[1],
        type = input[3],
        owner = 'noone',
...
RegisterNetEvent('tayer-territories:deleteTerritory')
AddEventHandler('tayer-territories:deleteTerritory', function()
    ...
    TriggerServerEvent('tayer-territories:deleteTerritory', input[1])
end)

RegisterNetEvent('tayer-territories:updateUI')
AddEventHandler('tayer-territories:updateUI', function(action, data)
...
local function isInTerritory(isDead)
    ...
    for k, v in pairs(territories) do
        ...
        if distance < tonumber(v.radius) and v.isTaking then
            lastTerritory = v.id
            TriggerServerEvent('tayer-territories:updateAttenders', lastTerritory, PlayerData.identifier, PlayerData.job.name, true, checkDead)
            return true, k
        end
    end
    if lastTerritory then
        TriggerServerEvent('tayer-territories:updateAttenders', lastTerritory, PlayerData.identifier, PlayerData.job.name, false, false)
        lastTerritory = nil
    end
    return false
end
                ...
                if distance < tonumber(v.radius) then
                    ...
                    if distance < 1.5 then
                        if IsControlJustPressed(0, 38) then
                            TriggerEvent('tayer-territories:infoMenu', {
                                id = v.id,
...
RegisterNetEvent('tayer-territories:infoMenu')
AddEventHandler('tayer-territories:infoMenu', function(terData)
        ...
        options = {
            {
                ...
                event = 'tayer-territories:captureClient',
                metadata = {
            ...
            },
            {
                ...
                event = 'tayer-territories:openStash',
                metadata = {
    ...
    if terData.type == 'dealer' then
        defaultContext.options[#defaultContext.options + 1] = {
            title = 'Sell Shop | 🌿',
            event = 'tayer-territories:sellList',
            args = {
                data = terData
            }
        }
    elseif terData.type == 'market' then
        defaultContext.options[#defaultContext.options + 1] = {
            title = 'Buy Shop | 🛒',
            event = 'tayer-territories:buyList',
            args = {
                data = terData
            }
        }
...
RegisterNetEvent('tayer-territories:letCount')
AddEventHandler('tayer-territories:letCount', function(data)
    ...
    if data.selected.type == 'buy' then
        TriggerServerEvent('tayer-territories:buyMarket', itemObject)
    elseif data.selected.type == 'sell' then
        TriggerServerEvent('tayer-territories:sellDealer', itemObject)
    end
end)

RegisterNetEvent('tayer-territories:sellList')
AddEventHandler('tayer-territories:sellList', function(args)
    ...
    for k, v in pairs(shared.itemsToSell) do
        ...
        itemList[item] = {
            ...
            event = 'tayer-territories:letCount',
            args = {
                selected = {
                    name = label,
                    key = item,
...
RegisterNetEvent('tayer-territories:buyList')
AddEventHandler('tayer-territories:buyList', function(terData)
...
RegisterNetEvent('tayer-territories:captureClient')
AddEventHandler('tayer-territories:captureClient', function(terData)
    ...
    TriggerServerEvent('tayer-territories:captureServer', currentTerritory.id, PlayerData.job.name, PlayerData.job.label, currentTerritory.name, currentTerritory.owner)
end)

RegisterNetEvent('tayer-territories:openStash')
AddEventHandler('tayer-territories:openStash', function(terData)
...
RegisterNetEvent('tayer-territories:progressBars')
AddEventHandler('tayer-territories:progressBars', function(type)
    if type == 'start' then
...
RegisterNetEvent('tayer-territories:captureProgress')
AddEventHandler('tayer-territories:captureProgress', function(terKey, terData)
        ...
        if distance < terData.radius then
            if not isDead then
                if terData.isTaking then
                    if progress < 60 then
                        TriggerEvent('tayer-territories:progressBars', 'start')
                        Wait(shared.capturing * 60000 / 60)
                        progress = progress + 1
                    else
                        TriggerEvent('tayer-territories:progressBars', 'stop')
                        TriggerServerEvent('tayer-territories:captureComplete', terData.id, PlayerData.job.name, PlayerData.job.label, terData.owner)
                        progress = 0
                ...
                progress = 0
                TriggerServerEvent('tayer-territories:endCapturing', lastTerritory)
                lastTerritory = nil
                break
            end
        else
...
RegisterCommand('territories', function(source, args, rawCommand)
        ...
        options = {
            {
                ...
                event = 'tayer-territories:listTerritories',
                metadata = {
                    'List of territories.'
                }
            },
    ...
    if shared.rankings then
        homePage.options[#homePage.options + 1] = {
            title = 'Rank List | 🏆',
            event = 'tayer-territories:listRankings',
            metadata = {
...
RegisterNetEvent('tayer-territories:listTerritories')
AddEventHandler('tayer-territories:listTerritories', function()
    local terCollection = {}
...
if shared.rankings then
    RegisterNetEvent('tayer-territories:listRankings')
    AddEventHandler('tayer-territories:listRankings', function()
        ESX.TriggerServerCallback('tayer-territories:fetchPoints', function(pointsCollection)
